...
class StamTest extends AnyFlatSpec with BeforeAndAfter with Logging with BeforeAndAfterAll {
  ...
  override def beforeAll: Unit = {
    ...
    val minioAdminClient = MinioAdminClient
      .builder()
      .endpoint(minio.getS3URL)
      .credentials(minio.getUserName, minio.getPassword)
      .build()
    ...
    minioClient.makeBucket(
      MakeBucketArgs.builder().bucket(BUCKET_NAME).region(Region.US_EAST_1.id()).build()
    )
  ...
  "stam test" should "tsam" in {
    ...
    implicit val streamProps: Properties =
      createStreamProps(bootstrapService, minio.getS3URL, dataDir.getAbsolutePath)
    val streams = createStreams(SOURCE_TOPIC_NAME, DEST_TOPIC_NAME)
    streams.enableS3Snapshot().start()

    val rangeExcl = Range(0, 20004)
    rangeExcl.foreach(v =>
      producer.send(new ProducerRecord(SOURCE_TOPIC_NAME, v.toString, v.toLong)).get()
    )
    ...
    val iterable = minioClient.listObjects(
      ListObjectsArgs
        .builder()
    ...
    val ls =
      StreamSupport.stream(iterable.spliterator(), false).collect(Collectors.toList()).asScala
    assert(ls.size == 1)
    val result = ls.head
    ...
    val bytes = minioClient
      .getObject(
        GetObjectArgs.builder().bucket(BUCKET_NAME).`object`(s"$kstoreName/.checkpoint").build()
      )
      .readAllBytes()
  ...
  def createConsumer[K](
    bootstrapServers: String,
    deserializer: Deserializer[K]
  ): KafkaConsumer[K, Long] = {
  ...
  def createProducer(bootstrapServers: String): KafkaProducer[String, Long] = {
    ...
    new KafkaProducer[String, Long](
      props,
      Serdes.stringSerde.serializer(),
      Serdes.longSerde.serializer()
    )
  }

  def createStreamProps(
    bootstrapServers: String,
    minioUrl: String,
    stateDirPath: String
  ): Properties = {
  ...
  def createStreams(sourceTopic: String, destTopic: String)(
    implicit streamsConfiguration: Properties
  ): KafkaStreams = {
