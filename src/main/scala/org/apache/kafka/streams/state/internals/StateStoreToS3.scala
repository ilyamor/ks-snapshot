...
object StateStoreToS3 extends Logging {
  ...
  (wrapped: SegmentedBytesStore, retainDuplicates: Boolean, windowSize: Long, props: Properties, segmentFetcher: T => List[RocksDB])
    ...
    override def init(context: StateStoreContext, root: StateStore): Unit = {
      ...
      val storageClientUploader: StorageUploader = if (UploaderUtils.isJavaStorageUploaderInstance(config.getClass(STATE_STORAGE_UPLOADER))) {
        StorageUploaderWrapperToJava(Utils.newInstance(config.getClass(STATE_STORAGE_UPLOADER), classOf[StorageUploaderJava]))
          .configure(props, prefixKey)
      } else {
        Utils.newInstance(config.getClass(STATE_STORAGE_UPLOADER), classOf[StorageUploader])
  ...
  object S3StateStoreConfig {
    ...
    private def CONFIG = new ConfigDef()
      ...
      .define(STATE_STORAGE_UPLOADER, Type.CLASS, classOf[UploadS3ClientForStore], (name: String, value: Any) => {
        value match {
