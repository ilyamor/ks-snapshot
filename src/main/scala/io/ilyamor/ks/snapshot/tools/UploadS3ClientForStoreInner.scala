...
case class UploadS3ClientForStoreInner private(config: S3StateStoreConfig, storeKey: String)
  ...
  def getCheckpointFile(
    ...
    Try {
      ...
      val res: ResponseInputStream[GetObjectResponse] =
        client.getObject(GetObjectRequest.builder().bucket(bucket).key(checkpointPath).build())
    ...
    }.map(new OffsetCheckpoint(_)).toEither
  }
  ...
  private def uploadArchive(archiveFile: File): Either[Throwable, String] = {
    ...
    Try({
      ...
      val createResponse: CreateMultipartUploadResponse =
        client.createMultipartUpload(createRequest)
